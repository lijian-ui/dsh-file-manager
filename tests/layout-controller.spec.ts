...
const explorerCol = (): HTMLElement => document.querySelector('[data-filemgr-explorer-col]') as HTMLElement
const previewCol = (): HTMLElement => document.querySelector('[data-filemgr-preview-col]') as HTMLElement
const floatingButton = (): HTMLButtonElement => document.querySelector('.filemgr-floating-expand') as HTMLButtonElement
...
describe('maximize (issue #315)', () => {
  ...
  it('switches to the fixed full-screen overlay on narrow rows', () => {
    ...
    expect(explorerCol().classList.contains('filemgr-maximized')).toBe(true)
    expect(previewCol().classList.contains('filemgr-maximized')).toBe(false)
    // The takeover grid is skipped in overlay mode: the fixed column covers.
    expect(grid()).toBe('240px minmax(0, 1fr) 0px 0px 260px')

    layout.update((prev) => ({ ...prev, maximized: null }))
    expect(explorerCol().classList.contains('filemgr-maximized')).toBe(false)
  })
