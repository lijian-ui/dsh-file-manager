/**
 * Raw-route integration test: the GET /filemgr/raw dispatch inside the
...
describe('GET /filemgr/raw', () => {
  it('streams workspace bytes with the derived mime and no-cache', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-route-')))
    ...
    const result = await request(row!.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}&path=assets/pic.png`)
    ...
    const encoded = await request(row!.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}&path=assets%2Fpic.png`)
  ...
  it('maps missing files to 404, .git and directories to 403, bad params to 400', async () => {
    ...
    const missing = await request(row.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}&path=nope.png`)
    expect(missing.status).toBe(404)
    const gitPath = await request(row.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}&path=.git/config`)
    expect(gitPath.status).toBe(403)
    const dirPath = await request(row.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}&path=.git`)
    expect(dirPath.status).toBe(403)
    const empty = await request(row.handler, 'GET', `/filemgr/raw?root=${encodeURIComponent(root)}`)
    expect(empty.status).toBe(400)
    // Other GET paths are still rejected with 405.
    const other = await request(row.handler, 'GET', '/filemgr/list')
  ...
  it('rejects non-loopback raw reads with 403 before touching the filesystem', async () => {
    ...
    const result = await request(row.handler, 'GET', '/filemgr/raw?root=%2Fw&path=a.png', {
  ...
  it('serves single byte ranges (206) and rejects unsatisfiable ones (416)', async () => {
    ...
    registerPanelRoutes(ctx as never, new FsService(gate), { status: async () => null } as never)
    const row = registrations.find((item) => item.kind === 'prefix')!
    const url = `/filemgr/raw?root=${encodeURIComponent(root)}&path=doc.pdf`
