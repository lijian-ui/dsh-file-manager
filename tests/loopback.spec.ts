/**
 * Loopback fence tests for the /filemgr routes: non-loopback clients get
...
describe('/filemgr loopback fence', () => {
  it('serves loopback JSON operations as before', async () => {
    ...
    const result = await drive(prefix.handler, '/filemgr/list', {
  ...
  it('rejects non-loopback GET /filemgr/raw with 403', async () => {
    ...
    const result = await drive(prefix.handler, '/filemgr/raw?root=%2Fw&path=a.txt', {
      method: 'GET',
      remoteAddress: '192.168.1.20',
      host: '192.168.1.10:3000',
    })
  ...
  it('rejects non-loopback SSE before gating the root or opening the stream', async () => {
    ...
    const result = await drive(sse.handler, '/filemgr/events?root=%2Fw', {
      method: 'GET',
      remoteAddress: '192.168.1.20',
      host: '192.168.1.10:3000',
    })
