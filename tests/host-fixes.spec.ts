...
describe('FsService symlink escape (C1)', () => {
  it('refuses to read/write/delete through a symlink pointing outside the root', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-gate-')))
  ...
  it('blocks .git writes and deletes (L4) but still lists/reads them', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-git-')))
  ...
  it('refuses delete spellings that normalize onto the root itself', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-rootdel-')))
    const root = join(dir, 'proj')
  ...
  it('reads oversized text files with a bounded read (cap preserved, size exact)', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-bigread-')))
    const root = join(dir, 'proj')
    await mkdir(root, { recursive: true })
...
describe('FsService.readRaw (markdown image route)', () => {
  it('returns raw bytes with a derived mime', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-raw-')))
    const root = join(dir, 'proj')
...
describe('GitService.discard path derivation (H1)', () => {
  ...
  it('refuses to delete a symlink that points outside the repo (C1)', async () => {
    const dir = await realpath(await mkdtemp(join(tmpdir(), 'filemgr-discard-')))
    const outsideDir = join(dir, 'outside')
