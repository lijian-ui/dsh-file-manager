...
describe('GET /filemgr/vendor/mermaid.js', () => {
  ...
  it('serves the build-copied asset with etag revalidation', async () => {
    ...
    const first = await request(row!.handler, 'GET', '/filemgr/vendor/mermaid.js')
    if (first.status === 404) {
    ...
    const revalidate = await request(row!.handler, 'GET', '/filemgr/vendor/mermaid.js', { 'if-none-match': etag! })
  ...
  it('stays behind the loopback fence', async () => {
    ...
    registerPanelRoutes(ctx as never, new FsService(gate), { status: async () => null } as never)
    const row = registrations.find((item) => item.kind === 'prefix')
    const lan = await request(row!.handler, 'GET', '/filemgr/vendor/mermaid.js', {}, '192.168.1.5')
